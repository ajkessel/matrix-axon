...
**Milestone 7a in flight** (PRs 1–3a of ~6 landed) — M6 (mutations) is complete and the post-M6 sequence was rethought (see `docs/mvp/implementation.md` "Milestone resequencing" + ADR 0022). **M7** is account lifecycle & auth, in three phases: **7a** the Matrix-account lifecycle (login/verify/recover/logout/delete) — which also folds in the interactive SAS verification deferred from M5 (the old "5c") as its *last* PR; **7b** the client↔axon bearer-token gate (was M8); **7c** sender-device trust. The interactive-verification work and `axon-crypto` remain a stub until 7a's final PR.

Landed so far in 7a: PR 1 (the account state machine), PR 2 (runtime login, `POST /v1/accounts/login`), PR 3 (logout, `POST /v1/accounts/{id}/logout`), and PR 3a (server-side homeserver discovery) — see the notes immediately below.
