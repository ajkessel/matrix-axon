...
- **Subtract all first-line label injections from `first_body_width` before wrapping body text.** Any label inserted into the header line after body text is wrapped (e.g. the `[thread root] ` label, 14 columns) causes the body to overflow the right border. Always compute the full first-line overhead — sender label + timestamp + any injected labels — and subtract the total from the width before calling body-wrapping functions.
