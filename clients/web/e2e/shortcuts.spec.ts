...
test('Ctrl-Shift-Y cycles the filter in the TUI’s order', async ({ page }) => {
  await openRoom(page)
  const activeChip = () => page.locator('.chip.active').innerText()

  expect(await activeChip()).toBe('All')
  for (const expected of ['DMs', 'Groups', 'Unread', 'Favorites', 'All']) {
...
test('ArrowUp on an empty composer edits your last message', async ({
  ...
  await expect(page.getByText('my last words')).toBeVisible()
  await expect(composer).toHaveValue('')
