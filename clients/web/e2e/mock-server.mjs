...
timeline.push({
  ...
  arrival_order: Date.now() - 86_400_000,
  type: 'm.room.member',
...
timeline.push({
  ...
  arrival_order: Date.now() - 86_390_000,
  type: 'm.room.topic',
...
for (const [index, encrypted] of [false, true, true, false].entries()) {
  ...
  timeline.push({
    ...
    arrival_order: GALLERY_BASE + index * 4_000,
    type: 'm.room.message',
    content: {
      msgtype: 'm.image',
...
const LIGHTBOX_OWN_IMAGE = {
  ...
  arrival_order: Date.now() - 2_400_000,
  type: 'm.room.message',
  content: {
    msgtype: 'm.image',
    body: 'own-image.png',
    ...
    [
      {
        ...
        arrival_order: Date.now() - 7_200_000,
        type: 'm.room.message',
