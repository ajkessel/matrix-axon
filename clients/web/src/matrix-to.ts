...
export function parseMatrixRoomReference(
  ...
  if (matrixTo !== null && isRoomMxid(matrixTo.mxid)) {
    return {
...
function isRoomMxid(mxid: string): boolean {
  return (
    (mxid.startsWith('!') || mxid.startsWith('#')) &&
    serverNameFromRoomReference(mxid) !== null
  )
}
...
function parseMatrixUriRoomReference(
  ...
  if (!isRoomMxid(roomIdOrAlias)) {
    return null
  }
  return {
    roomIdOrAlias,
    serverNames: queryValues(query, 'via'),
