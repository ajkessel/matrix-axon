 ...
 */
export function useFileDrop(onFile: (file: File, skipped: number) => void): {
  dragging: boolean
  handlers: {
  ...
  return {
    ...
    handlers: {
      ...
      onDrop(event) {
        ...
        event.preventDefault()
        reset()
        // One file at a time (ADR 0065): a multi-file drop takes the first, and
        // reports the rest so the UI can admit it dropped them.
        const files = event.dataTransfer?.files
        const file = files?.[0]
        if (files !== undefined && file !== undefined) {
          onFile(file, files.length - 1)
        }
      },
    },
  }
