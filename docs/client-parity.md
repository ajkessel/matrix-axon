...
| Save a displayed image to the device | n/a — client-side over the existing proxy | Done — download to disk | Done (ADR 0081) — share sheet where the platform takes files, transient anchor otherwise | Not started | `window.open` is banned repo-wide for the Tauri shell (M-W12), so the anchor is the only sanctioned path |
