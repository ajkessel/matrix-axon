...
| HEIC / HEIF images | n/a — the proxy passes the sender's bytes and type through untouched (`is_inline_safe`) | **Gap** — no HEIF decoder in the `image` crate; sniffs the format and says so, no render | **Gap** — Chromium and Gecko decode no HEIC; names the format and offers Download (ADR 0101), renders natively under WebKit | Not started | A shared limitation, not a web bug: whether to ship a decoder is the open question in ADR 0101, and it is a license decision (libheif is LGPL-3.0) more than a rendering one |
...
| Read receipts that cover a redaction-hidden event | n/a — client-side; the route already accepts any event id | n/a — `should_show_event` has no redaction filter, so a redacted event stays a receipt candidate and never blocks the target | Done (#339) — the receipt candidate set admits events hidden *only* by redaction, which the rendered timeline still hides | Not started | A trailing event redacted moments after it arrives otherwise pins the room's server-derived count forever (#337). TUI is immune structurally rather than by design — adding a hide-redacted setting there would reintroduce it |
...
| Matrix OAuth QR account acquisition | Done (ADR 0097) — authenticated, polling-based display/scan flows at `/v1/accounts/login/qr`; successful login is accepted only when Axon's new device is cross-signed | Planned — display QR, enter/display check code, poll, cancel | Done — unified `/accounts` acquisition with lazy QR display, camera/image scan, check-code handling, approval link, polling, reload resume, and cancellation | Not started | MAS/Synapse interoperability and black-box smoke remain the final integration slice after client UIs |
| Matrix OAuth QR device authorization | Done (ADR 0097) — account-scoped polling flows recheck current trust before releasing secrets and require authorization-server approval | Not planned — no concrete terminal grant use case | Done — unified `/accounts` grant UI with camera/image scan, QR display, check codes, approval link, polling, cancellation, flow-id-only reload recovery, and expected-account guidance | Not started | Element X/MAS interoperability and black-box smoke remain the final integration slice |
...
| Megolm key backup (originate + honesty) | Done (ADR 0098) — `AccountDto.backup` snapshot, recover auto-enable/join/export-resume, `POST /v1/accounts/{id}/backup/enable`, recover flatten + `redecrypt`/`backup_action` | Done — `/backup enable [user]` (masked key; empty Enter kicks upload) and `/status` snapshot; `/recover` unchanged | Done — account-card snapshot + badge, honest recover/login notices (`backup_action`, never "Keys recovered"), Enable backup (optional key; muted when already uploading), UTD "Recover keys" link to `/accounts` | Not started | Server GET cache of `exists_on_server` may lag another client; enable then 409s. No `account.backup` WS yet (#292). Recover does not decrypt pre-Axon history |
