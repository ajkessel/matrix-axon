...
| Room membership (leave/forget/invite/kick/ban/unban) | Done (M19b, ADR 0068) — `POST .../rooms/{room_id}/{leave,forget,invite,kick,ban,unban}` | Done (ADR 0074 TUI-M19-1) — slash commands `/leave`, `/part`, `/forget`, `/invite`, `/kick`, `/ban`, `/unban` | Partial (M19-W1) — `/leave`, `/part`, and `/forget`; member-list invite/kick/ban/unban remains | Not started | TUI exposes moderation as selected-room slash commands; web M19-W5 will add invite/kick/ban/unban |
| Room entry (join/knock/create) | Done (M19c, ADR 0068) — `POST .../rooms/{join,knock,dm}` and `POST .../rooms` | Not started | Partial (M19-W2/W3) — `/join`, `/knock`, Matrix room-link interception, opt-in browser `matrix:` handler, and visible find/join plus directory join-from-result; create-room/DM remains | Not started | Web W4 adds create-room/DM | 
