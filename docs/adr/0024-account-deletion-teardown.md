...
### Transitional guard: refuse `DELETE` of a config-provisioned account

A row deleted here is *gone*, so the next boot's unconditional `upsert_account` of
the `sync.account` provision would recreate it as a fresh `active` row — the
deletion would not survive a restart. Rather than tombstone a removed identity (a
schema + GC burden for a path that is itself slated for removal), `delete` **refuses
to initiate** on an account the running config still names (`matches_account`
against `config.account`), returning `409` with "remove it from `sync.account`
first". A row already `deleting` is exempt, so the boot reconcile always finishes an
in-flight teardown rather than wedging on it. This guard is deliberately
transitional: it becomes dead code the day config-based provisioning is retired
(below), at which point nothing resurrects anything and the guard can be deleted.
...
- **Partial / not yet closed (#24):** the *config-drop* half stays open — see the
  scope note in *Context*. A row dropped from `sync.account` is still `active` and
  this PR does not retire it; the transitional guard only stops the inverse footgun
  (a `DELETE` silently resurrected by provisioning).
- **Follow-up — retire config-based provisioning.** The principled close of the
  config-drop half is to stop boot from provisioning accounts at all, once runtime
  endpoints cover everything `sync.account` carries: first-login (`POST
  /v1/accounts/login`, landed) and the **recovery-key import** (`POST
  /v1/accounts/{id}/recover`, a later 7a PR — the current blocker, since the boot
  recovery-key path has no runtime equivalent yet). When that lands, deleting the
  `AccountProvision` surface makes `DELETE` durable by construction and the
  transitional guard above becomes deletable dead code. Tracked separately.
