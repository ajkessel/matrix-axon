...
- Both clients already have a single, debounced choke point where "this room
  was just read" is decided: `App::note_room_read` → `spawn_marker_put` in
...
fully-read marker set to the same event.
...
PUT (`spawn_marker_put` in the TUI, the flush path behind `advanceReadMarker`
