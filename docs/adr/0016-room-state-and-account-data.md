...
timeline), so nothing is lost; these are ordinary tables the sync handlers keep
current on every write, holding the resolved current value a room-summary or
read-marker read consults.
