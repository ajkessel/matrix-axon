...
> **Status:** Milestones 1–6 have shipped; **M7 onward is the forward plan.** This document tracks the _plan_, not live progress — for what's actually built and in flight, see `AGENTS.md` "Current state" (kept here to avoid two sources of truth drifting). The post-M6 resequencing is summarized in the table near the end.
...
| **7c** Sender-device trust & content authentication | — (new)                             | Evaluate + expose whether _other_ senders' devices are cross-signed; delivers the verification-bundle API the tech-spec already promises.       |
...
- No full OAuth 2.0 server. Bearer tokens via CLI only. _(Post-MVP: revisited by M14 — ADR 0054 proposes axon become its own OAuth 2.0 authorization server behind the existing `TokenVerifier` seam; in design, no code landed.)_
