...
Run axon against the local servers. **Isolate it from `.env`** — `axon-server`
loads `.env` via dotenvy from the working dir upward, and the project `.env`
points at a *remote* account + recovery key. Running the built binary from a
...
AXON_SYNC__ACCOUNT__PASSWORD=hunter2secret \
RUST_LOG=info,axon_sync=debug \
  "$BIN"
```

Watch for `logged in` → `sync service started` → `persisted event … event_type="m.room.message"`,
then Ctrl-C and confirm the row landed:
...
1. **Fresh axon device, no recovery key** — run axon as in step 3 but wipe its
   ...
   ```
2. **Restart axon with the recovery key** from the seeder:
   ```sh
   # …same run, plus:  AXON_SYNC__ACCOUNT__RECOVERY_KEY="<recovery_key from the JSON>"
   ```
   Watch for `imported key backup via recovery key`, then `re-decrypted UTD`
...
> Synapse. It is **not** the key in `.env`, which belongs to a remote account.
