...
impl Ctx {
    ...
    pub fn spawn_tui(&self, scenario: &str, base_url: &str) -> anyhow::Result<PtyDriver> {
        ...
        let args = vec!["--base-url".to_owned(), base_url.to_owned()];
...
pub async fn run(profile: &str, filter: Option<&str>) -> anyhow::Result<i32> {
    if profile != "stub" {
        bail!("unknown profile {profile:?}; the only S1 TUI profile is `stub`");
    }
    ...
    eprintln!(
        "smoke(tui): run {run_id}, profile=stub, {} scenario(s), binary {}",
        selected.len(),
        ctx.bin.display()
    );
